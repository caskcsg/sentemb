...
This repository contains the code and pre-trained models for our COLING 2022 paper [Smoothed Contrastive Learning for Unsupervised Sentence Embedding](https://arxiv.org/abs/2109.04321)(GS-InfoNCE).
...
## Quick Links
  ...
  - [Use gsInfoNCE with Huggingface](#use-gsInfoNCE-with-huggingface)
  - [Train gsInfoNCE](#train-gsInfoNCE)
...
|              Model              | Avg. STS |
|:-------------------------------|:--------:|
|  [unsup-gsInfoNCE-bert-base-uncased](https://huggingface.co/ffgcc/gsInfoNCE-bert-base-uncased) |   77.63 |
| [unsup-gsInfoNCE-bert-large-uncased](https://huggingface.co/ffgcc/gsInfoNCE-bert-large-uncased) |   78.96  |
|    [unsup-gsInfoNCE-roberta-base](https://huggingface.co/ffgcc/gsInfoNCE-roberta-base)    |   77.74  |
|    [unsup-gsInfoNCE-roberta-large](https://huggingface.co/ffgcc/gsInfoNCE-roberta-large)   |   79.21  |

## Use gsInfoNCE with Huggingface
...
tokenizer = AutoTokenizer.from_pretrained("ffgcc/gsInfoNCE-bert-base-uncased")
model = AutoModel.from_pretrained("ffgcc/gsInfoNCE-bert-base-uncased")
...
In the following section, we describe how to train a gsInfoNCE model by using our code.
...
python evaluation.py \
    --model_name_or_path ffgcc/gsInfoNCE-bert-base-uncased \
    --pooler cls \
    --task_set sts \
    --mode test
```
...
+-------+-------+-------+-------+-------+--------------+-----------------+-------+
| 70.12 | 82.57 | 75.21 | 82.89 | 80.23 |    79.70     |      72.70      | 77.63 |
+-------+-------+-------+-------+-------+--------------+-----------------+-------+
```

Arguments for the evaluation script are as follows,

* `--model_name_or_path`: The name or path of a `transformers`-based pre-trained checkpoint. You can directly use the models in the above table, e.g., `ffgcc/gsInfoNCE-bert-base-uncased`.
...
We provide example training scripts for gsInfoNCE. In `run.sh`, we provide a single-GPU (or CPU) example.
...
* `--random_size`: The size of negative examples.
* `--random_std`: standard deviations.
...
Please cite our paper if you use gsInfoNCE in your work:
