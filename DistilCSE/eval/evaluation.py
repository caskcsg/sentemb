...
class CSKD(nn.Module):
    def __init__(self,model_name,config):
        super().__init__()
        self.student_model=AutoModel.from_pretrained(model_name,config=config)
        if config.num_hidden_layers==6 or config.num_hidden_layers==12:
            self.linear = nn.Linear(768,1024)
        elif config.num_hidden_layers==4:
            self.linear = nn.Linear(312,1024)
...
def main():
    ...
    #########
    dic_model_path={
        'nhl4':'../model/2nd_General_TinyBERT_4L_312D', #path to 4L
        'nhl6':'../model/2nd_General_TinyBERT_6L_768D', #path to 6L
        'Roberta':'roberta-base',
    }
    last_model=''
    for key in dic_model_path.keys():
        if key in args.model_name_or_path:
            last_model=dic_model_path[key]
    if last_model=='':
        last_model='bert-base-uncased'
    
    config=AutoConfig.from_pretrained(last_model)
    model=AutoModel.from_pretrained(last_model,config=config)
